...
declaration without \texttt{\textbackslash leanok} is part of the proposed implementation.  The
project contains type-correct declaration stubs for the results in this blueprint; declarations
without \texttt{\textbackslash leanok}, including the two final challenge statements, still have
placeholder proofs.
\end{remark}
...
\uses{thm:dhj}
For every $k\geq3$ and every $\delta>0$ there is a natural number $N=N(k,\delta)$ such
...
The Lean proof chooses, for each direction, a coordinate supplied by the \texttt{proper} field.
Evaluating an equality of parameter words at that coordinate recovers equality in the chosen
direction, and function extensionality concludes the result.
...
lines of $\eta\to\alpha$ and lines contained in $V$.  The Lean construction replaces each variable
direction of $V$ by the corresponding fixed or variable coordinate of the parameter line, and its
evaluation is exactly the composite of the two evaluations.  For the inverse, Lean chooses a
parameter word representing each point of an ambient line and one proper coordinate for every
direction of $V$.  Reading the ambient line at these coordinates recovers a parameter line; its
properness follows because a variable coordinate of the ambient line cannot be fixed by $V$.

Nested parameter subspaces are composed by substituting the inner generating word for every
variable direction of the outer one.  The resulting evaluation satisfies
$(V\circ W)(x)=V(W(x))$; this is the form used to pull line-free and containment hypotheses back
through every density increment.
Subspaces on disjoint coordinate blocks are concatenated in the same parameterized form, and their
evaluation is the concatenation of their separate evaluations.
For finite blocks, Lean then reindexes the two sums by \texttt{finSumFinEquiv}; the resulting
\texttt{concatFin} subspace has the expected dimensions and exposes separate evaluation formulas
on its left and right coordinate blocks.
...
\lean{Finset.dens}
\mathlibok
For a finite set $A\subseteq\mathbb W(\alpha,\iota)$ use mathlib's uniform density
\[
  \dens(A)=\frac{\card A}{\card\alpha^{\card\iota}}\in\mathbb Q_{\geq0}.
\]
For a subspace $V$, define the project-specific relative density by pulling back to its parameter
cube:
\[
  \dens_V(A)=\dens\{x:V(x)\in A\}.
\]
\end{definition}

The injectivity of $V$ makes this equal to
$\card(A\cap[V])/\card[V]$.  Quantitative arguments cast the nonnegative rational density to
$\mathbb R$ only when real arithmetic is needed.  The cardinal inequalities in the public API are
converted to density inequalities once, using the cardinality of the full function type.
...
The implementation uses mathlib's \texttt{Equiv.sumArrowEquivProdArrow} for the underlying
equivalence and defines only the convenient concatenation and fiber wrappers.
...
In Lean the full-cube identity is a short consequence of \texttt{Finset.expect\_product}; the
proof rewrites densities as indicator expectations and reindexes the product cube through
\texttt{Equiv.sumArrowEquivProdArrow}.
\end{proof}
...
The Lean proof expresses the upper bound as $b+(1-b)\mathbf 1_H$, averages the indicator, and
uses $b<1$ to divide by $1-b$.
\end{proof}
...
In Lean, the noncomputable bound \texttt{focusBound} is the least dimension obtained from this
coloring theorem for the finite profile type.  Its specification is upward closed: a line in the
chosen initial coordinates is extended by fixing every added coordinate.  This permits the theorem
to use the bound solely through \texttt{focusBound\_spec} and to transport arbitrary finite
alphabets, profile indices, and color types through their equivalences with \texttt{Fin}.
\end{proof}
...
\begin{proof}
We first prove the infinite ordered-block statement from the strong Hindman theorem.  Regard finite
words in $\N$ as the free monoid under concatenation, and begin with the stream of singleton words
$[0],[1],\ldots$.  Every finite product from this stream is a nonempty strictly increasing word.
For each $c\in C$, let $A_c$ consist of the nonempty strictly increasing words $w$ for which
$\chi(\operatorname{set}(w))=c$.  These finitely many sets cover all finite products of the
singleton stream, so strong Hindman supplies a stream $(b_i)_{i\in\N}$ and a color $c$ such that
every finite product of the $b_i$ belongs to $A_c$.

Put $B_i=\operatorname{set}(b_i)$.  Applying the conclusion to $b_i$ shows that $B_i$ is nonempty,
and applying it to $b_i b_j$ for $i<j$ shows that every element of $B_i$ is smaller than every
element of $B_j$.  For a nonempty finite index set $I$, concatenate the $b_i$ with $i\in I$ in
increasing order.  This is again a finite product, and its underlying set is
$\bigcup_{i\in I}B_i$, so every member of $\FU(B_1,\ldots,B_m)$ has color $c$.

It remains to make the ambient set finite.  Suppose contrariwise that every $L$ admits a bad
coloring $\chi_L$ of $\operatorname{Fin}L$.  Given a finite family $s$ of finite subsets of $\N$,
choose an initial segment containing the support of $s$ and color every member of $s$ by the
corresponding $\chi_L$.  The Rado selection principle stitches these compatible finite requests
into a global coloring $\chi$ of the finite subsets of $\N$.  Apply the infinite result to $\chi$
and then ask for agreement on the finite family of all unions of the first $m$ blocks.  Rado gives
one local bad coloring agreeing on that entire family, but the transported blocks make all those
unions monochromatic, a contradiction.

In Lean, \texttt{BlockWord.mem\_fp\_singletonStream} proves that the initial finite products are
increasing, while \texttt{BlockWord.select\_mem\_fp} and
\texttt{BlockWord.mem\_toFinset\_select} implement the Boolean selection of an arbitrary nonempty
union.  The finite compactness step uses \texttt{Finset.rado\_selection\_subtype}.
The theorem \texttt{exists\_mono\_in\_high\_dimension} transports a witness along the initial
order embedding $\operatorname{Fin}L\hookrightarrow\operatorname{Fin}n$.  Finally,
\texttt{FiniteUnions.bound} is the least resulting witness for \texttt{Fin colors}, and
\texttt{FiniteUnions.bound\_spec} transports the colors through
\texttt{Fintype.equivFin}; downstream proofs therefore use only its upward-closed specification.
\end{proof}
...
\begin{proof}
Color each line by whether it belongs to $\cL$, and apply Theorem~\ref{thm:graham-rothschild}
with two colors.  The common color of the resulting subspace gives the two alternatives.
\end{proof}
...
The Lean development first packages the $m=1$ case in
\texttt{densityOneBound}: it chooses the threshold supplied by $\mathsf{DHJ}_k$ and reindexes a
line viewed as a unit-dimensional subspace to one with parameter type \texttt{Fin 1}.  To ensure
that the suffix line type is nonempty, the successor step takes the suffix dimension to be the
maximum of $1$ and the threshold at density $\delta/2$.  It enumerates the finite line type through
the injective map from lines to their index words; the separate cardinal bound by
$(k+1)^M$ is also available.
For the degenerate alphabet $k=0$, it instead repeats the $m$ parameter coordinates across the
ambient cube; there are no parameter words, so this subspace is vacuously contained in every
family.  In the positive-alphabet branch, Lean converts between the cardinal hypothesis and
\texttt{Finset.dens}, then applies the threshold-averaging lemma to obtain the dense prefix family.
It then uses a finite fiber-averaging lemma to select one suffix line occurring on at least its
average relative density of those prefixes, namely at least $\delta/(2\#\Lines([k]^M))$.
Each selected line is obtained by applying the one-dimensional density assertion to its dense
suffix fiber.  The induction produces a threshold valid in every larger dimension; Lean defines
\texttt{densityBound} by \texttt{Nat.find} from this eventual statement and exposes
\texttt{densityBound\_spec} as its downstream interface.
...
\label{eq:main-constants}
 ...
 \eta=\min\left\{\frac{\delta\theta}{48},\frac\theta4,\frac\delta6\right\},\qquad
 \gamma=\min\left\{\frac{\delta\eta^2}{k},\frac{\eta^2}{2},3\eta\right\}.
\end{equation}
Every denominator is positive.  We record positivity and the elementary consequences
$\eta<\theta/2$, $\eta\leq\delta/6$, $\gamma\leq\eta^2/2$, and $\gamma\leq3\eta$ separately, so
later proofs can use them without unfolding the constants.
In Lean, the dimension is selected transparently as the least threshold supplied by the density
Hales--Jewett assertion when it is available (and is otherwise set to $1$); the selected threshold
is increased by one so that the power difference in the denominator is always positive.  Leastness
makes the selected dimension antitone in the density threshold, which is used for the uniform
increment below; the power difference itself is monotone in that dimension.

\begin{lemma}[Positivity of $\theta$]
\label{lem:theta-pos}
\lean{DensityHalesJewett.Parameters.θ_pos}
If $k\geq2$ and $\delta>0$, then $\theta>0$.
\end{lemma}

\begin{lemma}[Positivity of $\eta$]
\label{lem:eta-pos}
\lean{DensityHalesJewett.Parameters.η_pos}
If $k\geq2$ and $\delta>0$, then $\eta>0$.
\end{lemma}

\begin{lemma}[Positivity of $\gamma$]
\label{lem:gamma-pos}
\lean{DensityHalesJewett.Parameters.γ_pos}
If $k\geq2$ and $\delta>0$, then $\gamma>0$.
\end{lemma}

\begin{lemma}[$\eta$ is small relative to $\theta$]
\label{lem:eta-lt-theta-div-two}
\lean{DensityHalesJewett.Parameters.η_lt_θ_div_two}
If $k\geq2$ and $\delta>0$, then $\eta<\theta/2$.
\end{lemma}

\begin{lemma}[$\eta$ is small relative to $\delta$]
\label{lem:eta-le-delta-div-six}
\lean{DensityHalesJewett.Parameters.η_le_δ_div_six}
We have $\eta\leq\delta/6$.
\end{lemma}

\begin{lemma}[$\gamma$ is bounded by $\eta^2/2$]
\label{lem:gamma-le-eta-sq-div-two}
\lean{DensityHalesJewett.Parameters.γ_le_η_sq_div_two}
We have $\gamma\leq\eta^2/2$.
\end{lemma}

\begin{lemma}[$\gamma$ is bounded by $3\eta$]
\label{lem:gamma-le-three-eta}
\lean{DensityHalesJewett.Parameters.γ_le_three_mul_η}
We have $\gamma\leq3\eta$.
\end{lemma}
...
\uses{lem:uniform-fibers,cor:gr-two-color,lem:threshold-average,lem:lines-equiv,lem:eta-pos,lem:eta-le-delta-div-six}
Let $m\geq m_0$ and
...
\uses{lem:correlated-fibers,lem:threshold-average,lem:eta-pos,lem:eta-lt-theta-div-two}
...
\lean{DensityHalesJewett.IsInsensitive}
...
\lean{DensityHalesJewett.IsInsensitive.compl}
...
\uses{lem:many-lines,def:insensitive,lem:lines-equiv,lem:theta-pos,lem:eta-pos,lem:eta-lt-theta-div-two,lem:eta-le-delta-div-six}
...
\uses{lem:large-insensitive-intersection,lem:insensitive-boolean,lem:eta-le-delta-div-six,lem:gamma-le-eta-sq-div-two,lem:gamma-le-three-eta}
...
\lean{DensityHalesJewett.IsInsensitive.exists_disjoint_subspaces}
...
\lean{DensityHalesJewett.IsInsensitive.exists_disjoint_subspaces_iInter}
...
The Lean proof constructs this line coordinatewise, using a variable coordinate exactly for a
$0\to1$ transition; finite case analysis verifies both endpoint words.  It also records separately
that binary support is injective, since membership of a coordinate in the support determines its
binary letter.
\end{proof}
...
The Lean proof supplies the required estimate directly.  From the central-binomial recurrence it
proves by induction that
\[
 \left(\binom{2m}{m}/4^m\right)^2(3m+1)\leq1,
 \qquad\text{hence}\qquad
 \binom{2m}{m}/4^m\leq(3m+1)^{-1/2}.
\]
Pascal's identity bounds the normalized middle layer in odd dimension by the preceding even
central layer.  Since $(3m+1)^{-1/2}\to0$, this gives the eventual strict inequality used after
Sperner's theorem.  A hypothetical line-free family has injective binary-support image, and the
strict-comparability characterization shows that image is an antichain.
...
\lean{DensityHalesJewett.Parameters.γ_mono_lowerBound}
\uses{lem:gamma-pos}
...
\[
  \gamma(k,\rho)\geq\gamma(k,\delta_0)>0.
\]
\end{lemma}

\begin{proof}
The Lean definition chooses the least valid dimension at each density.  Its leastness makes the
dimension antitone, and monotonicity of the power difference then gives
\[
 \theta(\rho)\geq
 \theta(\delta_0).
\]
The capped definitions of $\eta$ and $\gamma$ are monotone in their positive arguments, so
$\gamma(k,\rho)\geq\gamma(k,\delta_0)>0$.
\end{proof}
...
floor $\delta_0>0$.  Let $\varepsilon=\gamma(k,\delta_0)/2$ and choose
...
Assume $k\geq3$, let $m=\DHJ(k,\delta/2)$, and put $K=k^m$.  Choose the public bound to be
at least $2K/\delta$.  By
...
\item \texttt{Szemeredi.lean}: digital encoding and the final arithmetic-progression
...
The implementation should reuse the following existing mathlib interfaces rather than duplicate
them: \texttt{Finset.dens} and its Boolean API, \texttt{Finset.expect} together with
\texttt{Finset.expect\_product}, \texttt{Equiv.sumArrowEquivProdArrow},
\texttt{Combinatorics.Subspace.exists\_mono\_in\_high\_dimension},
\texttt{IsAntichain.sperner}, and \texttt{Nat.ofDigits}.  The multidimensional coloring theorem
does not subsume the required Graham--Rothschild theorem, since the latter colors line structures
rather than points of a cube.  Similarly, \texttt{Nat.ofDigits} supplies the arithmetic operation
and basic lemmas but not the bounded fixed-length equivalence needed below.
